...
export default function PaintPicker() {
  ...
  async function handleAdd(paintId: string) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(await extractError(res, "Failed to add paint"));
      }
      setPaints((prev) => prev.map((paint) => (paint.id === paintId ? { ...paint, owned: true } : paint)));
    } catch (err) {
      ...
      <ul className="max-h-[60vh] space-y-2 overflow-y-auto">
        {filtered.map((paint) => (
          <li key={paint.id} className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 px-3 py-2">
            ...
            {paint.owned ? (
              <span className="flex items-center gap-1 rounded-lg border border-green-400/30 bg-green-900/20 px-3 py-1.5 text-sm text-green-300">
              ...
              <Button
                ...
                onClick={() => void handleAdd(paint.id)}
        ...
        {filtered.length === 0 && <p className="text-sm text-blue-100/50">No paints match your search.</p>}
      </ul>
    </div>
  );
}
